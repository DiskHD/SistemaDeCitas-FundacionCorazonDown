...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Recepcionista · Todas las citas del sistema</p>
        </div>
        <a href="{{ route('recepcionista.citas.create') }}" class="inline-flex items-center rounded-lg bg-secondary px-5 py-2.5 text-sm font-semibold text-white hover:bg-secondary-dark shadow-sm transition-all hover-lift">
...
<!-- Filtros -->
<form method="GET" action="{{ route('recepcionista.citas.index') }}">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div class="lg:col-span-2">
                ...
                <input
                    ...
                    placeholder="Nombre, tutor o teléfono..."
                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm shadow-sm focus:border-secondary focus:outline-none focus:ring-2 focus:ring-secondary focus:ring-opacity-20"
                >
            </div>
            ...
            <!-- Pago -->
            <div>
                <label for="payment_status" class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">
                    Pago
                </label>
                <select
                    id="payment_status"
        ...
        <div class="flex flex-wrap gap-3 mt-4 pt-4 border-t border-gray-200">
            ...
            <a href="{{ route('recepcionista.citas.index') }}" class="inline-flex items-center rounded-lg bg-gray-100 px-5 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200 transition-all">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
@if($appointments->isEmpty())
    <div class="bg-white rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
        ...
        <a href="{{ route('recepcionista.citas.index') }}" class="inline-flex items-center rounded-lg bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200 transition-all">
            Ver todas las citas
        </a>
    </div>
@else
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-1 space-y-0.5">
                                    <div>Edad: {{ $c->patient->edad ?? '-' }} · Tel: {{ $c->patient->telefono_tutor ?? '-' }}</div>
                                    <div>Tutor: {{ $c->patient->nombre_tutor ?? '-' }}</div>
                                    <div>Dom: {{ $c->patient->domicilio ?? '-' }}</div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-4">
                                ...
                                @if($c->therapist?->tipoTerapeutaLabel())
                                    <div class="text-xs text-gray-500 mt-0.5">
                                        {{ $c->therapist->tipoTerapeutaLabel() }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <!-- Pago -->
                            <td class="px-4 py-4 whitespace-nowrap">
                                <form action="{{ route('recepcionista.citas.payment', $c) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                            ...
                            <td class="px-4 py-4 whitespace-nowrap">
                                <div class="flex flex-wrap gap-2">
                                    @if($c->status === 'pendiente')
                                        <a href="{{ route('recepcionista.citas.edit', $c) }}" class="inline-flex items-center rounded-lg bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-700 border border-amber-200 hover:bg-amber-100 transition-all">
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                            Editar
                                        </a>
                                        <form action="{{ route('recepcionista.citas.cancel', $c) }}" method="POST" class="inline" onsubmit="return confirm('¿Cancelar esta cita?')">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="inline-flex items-center rounded-lg bg-primary-light px-3 py-1.5 text-xs font-semibold text-primary-dark border border-primary hover:bg-primary hover:text-white transition-all">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                </svg>
                                                Cancelar
