...
<head>
    ...
    <title>Mis Citas — Terapeuta | FCD</title>
    <style>
        :root {
            ...
            --text: #333; --muted: #6b7280; --radius: 10px; --shadow: 0 2px 8px rgba(0,0,0,.08);
        }
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .navbar-brand .dot { width: 28px; height: 28px; background: rgba(255,255,255,.2); border: 2px solid rgba(255,255,255,.45); border-radius: 50%; display: flex; align-items: center; justify-content: center; }
        .navbar nav { display: flex; align-items: center; gap: .8rem; }
        ...
        .page-title .subtitle { font-size: .83rem; color: var(--muted); }
        .alert { padding: .8rem 1.1rem; border-radius: var(--radius); margin-bottom: 1rem; font-size: .88rem; }
        ...
        .fg input, .fg select { padding: .5rem .75rem; border: 1.5px solid var(--border); border-radius: 7px; font-size: .88rem; background: #fafafa; }
        .fg input:focus, .fg select:focus { outline: none; border-color: var(--green); box-shadow: 0 0 0 3px rgba(137,204,49,.12); }
        ...
        .results-bar { font-size: .82rem; color: var(--muted); margin-bottom: .75rem; }
        ...
        .desc-cell { max-width: 160px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        ...
        .pg-disabled { color: #ccc; background: #fafafa; }
    </style>
</head>
<body>
<header class="navbar">
    <div class="navbar-brand">
        <span class="dot">❤</span> FCD — Terapeuta
    </div>
    <nav>
...
<div class="container">
    <div class="page-title">
        <div>
            <h2>🩺 Mis Citas</h2>
            <div class="subtitle">Terapeuta · Solo mis citas asignadas</div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('terapeuta.citas.index') }}">
        <div class="filter-bar">
            ...
            <div class="filter-actions">
                ...
                <a href="{{ route('terapeuta.citas.index') }}" class="btn btn-ghost">✕</a>
            </div>
        </div>
    </form>
    ...
    @if($appointments->isEmpty())
        <div class="empty">
            ...
            <a href="{{ route('terapeuta.citas.index') }}" class="btn btn-ghost" style="margin-top:.9rem">Ver todas</a>
        </div>
    @else
    <div class="table-wrap">
        <table>
            <thead>
                <tr><th>#</th><th>Paciente</th><th>Fecha</th><th>Hora</th><th>Descripción</th><th>Estado</th><th>Acción</th></tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($c->status === 'pendiente')
                            <form action="{{ route('terapeuta.citas.complete', $c) }}" method="POST" style="display:contents" onsubmit="return confirm('¿Marcar como completada?')">
                                @csrf @method('PATCH')
    ...
    <div class="pagination">
        @if($appointments->onFirstPage()) <span class="pg-disabled">‹</span> @else <a href="{{ $appointments->previousPageUrl() }}">‹</a> @endif
        @foreach($appointments->getUrlRange(1, $appointments->lastPage()) as $page => $url)
            @if($page == $appointments->currentPage()) <span class="pg-active">{{ $page }}</span>
            @else <a href="{{ $url }}">{{ $page }}</a> @endif
        @endforeach
        @if($appointments->hasMorePages()) <a href="{{ $appointments->nextPageUrl() }}">›</a> @else <span class="pg-disabled">›</span> @endif
    </div>
    @endif
    @endif
</div>
